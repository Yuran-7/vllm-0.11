...
| 阶段 | 耗时 | 占比 | 关键操作 |
|------|------|------|---------|
| **参数解析** | 3s | 6% | CLI 参数解析、环境变量读取 |
| **模型加载** | 6s | 12% | 读取 safetensors、传输到 GPU |
| **torch.compile** | 26s | 52% | Dynamo 字节码转换、Inductor 编译 |
| **KV Cache 分配** | 1s | 2% | GPU 内存分析、block 分配 |
| **CUDA Graph 捕获** | 6s | 12% | 捕获 102 个图（67 prefill + 35 decode） |
| **服务初始化** | 8s | 16% | FastAPI 路由注册、组件初始化 |
| **总计** | **50s** | **100%** | 从启动到服务就绪 |

### 性能优化建议

#### 1. 减少 torch.compile 时间
- **缓存复用**：首次启动后缓存在 `~/.cache/vllm/torch_compile_cache/`
- **后续启动**：跳过编译直接加载缓存（约 5s）
- **禁用编译**（如果吞吐量要求不高）：
  ```bash
...
| 组件 | 文件路径 | 行数 | 功能 |
|------|---------|------|------|
| **命令行入口** | `vllm/entrypoints/openai/api_server.py:1942` | 12 | `if __name__ == "__main__"` |
| **服务器启动** | `vllm/entrypoints/openai/api_server.py:1877` | 63 | `run_server()`, `run_server_worker()` |
| **引擎创建** | `vllm/entrypoints/openai/api_server.py:152` | 88 | `build_async_engine_client()` |
| **AsyncLLM 初始化** | `vllm/v1/engine/async_llm.py:54` | 132 | `AsyncLLM.__init__()` |
| **EngineCore 初始化** | `vllm/v1/engine/core.py:62` | 174 | `EngineCore.__init__()` |
| **KV Cache 初始化** | `vllm/v1/engine/core.py:内部` | - | `_initialize_kv_caches()` |
| **模型执行器** | `vllm/v1/executor/gpu_executor.py` | - | `GPUExecutor` |
| **调度器** | `vllm/v1/core/sched/scheduler.py` | - | `V1Scheduler` |
| **FastAPI 应用** | `vllm/entrypoints/openai/api_server.py:1511` | 94 | `build_app()` |
| **应用状态初始化** | `vllm/entrypoints/openai/api_server.py:1606` | 244 | `init_app_state()` |
| **HTTP 服务启动** | `vllm/entrypoints/launcher.py` | - | `serve_http()` |


### IPC 通信机制

**通信协议**: ZMQ (Zero Message Queue)

**消息类型**:
1. **EngineCoreRequest**: 推理请求（add_request, abort_request）
2. **EngineCoreOutputs**: 推理输出（token ids, logprobs）
3. **UtilityRequest**: 工具请求（get_model_config, do_log_stats）
4. **UtilityResult**: 工具响应
...
engine_client.add_request(request_id, prompt, params)
    ↓
[ZMQ Socket] → IPC 序列化（msgpack）
    ↓
EngineCore 接收请求 → 加入调度队列
    ↓
调度器分配资源 → 执行推理
    ↓
生成输出 → 序列化（msgpack）
